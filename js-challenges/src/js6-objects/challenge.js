...
export const getUserAddress = (user) => {
  let label = user.address;
  return `${label.line1} ${label.line2} ${label.city} ${label.postcode}`;
};
...
export const setSafeAllergens = (customer, allergenList) => {
  let safeAllergens = allergenList.filter(
...
export const mergeFurniture = (furnitureLocationData, furnitureProductData) => {
  let merged = { ...furnitureLocationData, ...furnitureProductData };
  return merged;
};
